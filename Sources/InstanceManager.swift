        ...
        case "paper":
            ...
            if let data = APIutils().fetchAPIData(using: apiURL) {
                if let latestBuild = APIutils().findLatestBuild(from: data) {
                    let _ = downloadServerSoftware(type: type.lowercased(), version: innerVersion, instanceName: instanceName, build: latestBuild)
                    createStartScript(instanceName: instanceName, instanceType: type, instanceVersion: innerVersion, instanceBuild: latestBuild, memory: memory ?? 1024, nogui: nogui)
                } else {
